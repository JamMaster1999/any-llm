...
class ChatCompletionMessage(OpenAIChatCompletionMessage):
    ...
    """Images generated by the model, as OpenAI-style ``image_url`` content blocks.

    The OpenAI chat completion schema has no field for generated images, so this mirrors
    the shape OpenRouter and LiteLLM use, which is also the shape any-llm accepts for
    image input:
