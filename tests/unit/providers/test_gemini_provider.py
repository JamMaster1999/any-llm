...
@pytest.mark.asyncio
async def test_completion_with_text_response_format_is_no_op() -> None:
    api_key = "test-api-key"
    model = "gemini-pro"
    messages = [{"role": "user", "content": "Hello"}]
    ...
    with mock_gemini_provider() as mock_genai:
        ...
        assert generation_config.response_mime_type is None
        assert generation_config.response_schema is None


@pytest.mark.asyncio
async def test_completion_with_unsupported_dict_response_format_raises() -> None:
    api_key = "test-api-key"
    model = "gemini-pro"
    messages = [{"role": "user", "content": "Hello"}]
    ...
    with mock_gemini_provider():
        provider = GeminiProvider(api_key=api_key)
        with pytest.raises(ValueError, match="Unsupported response_format type: unsupported"):
            await provider._acompletion(
                CompletionParams(
                    model_id=model,
                    messages=messages,
...
def test_convert_response_wraps_pcm_audio_as_wav(rate: int) -> None:
    """TTS returns headerless PCM whose sample rate exists only in the mime type; the message
    carries a playable WAV with every audio part in order."""
...
def test_streaming_completion_keeps_pcm_audio_raw() -> None:
    """A streamed piece cannot carry a WAV header, since the total length is unknown, so it stays
    raw PCM like OpenAI's pcm16 stream; parts within one chunk are joined in order."""
